...
- wait()和notify()方法只能在synchronized块中使用
- 将notify()方法放在wait()方法之前调用，可能会导致线程永远等待，因为notify()
  方法会唤醒一个正在等待的线程，但如果没有线程在等待，那么notify()方法就会被忽略，导致线程永远等待。

- await()和signal()方法只能在Lock对象的条件变量上使用
- 将signal()方法放在await()方法之前调用，可能会导致线程永远等待，因为signal()
  方法会唤醒一个正在等待的线程，但如果没有线程在等待，那么signal()方法就会被忽略，导致线程永远等待。

- park()和unpark()方法可以在任何地方使用
